...
#define SYNCHRONIZED_OBJ(obj) \
	...
	INJECT_VARIABLE(std::lock_guard<std::mutex>,            \
						obfuscatedLockName(Sync::Synchronized_OBJ<decltype(obj)>(obj, obfuscatedMutexName).GetMutex())) 

	template<class T>
	class Synchronized_OBJ
	{
	public:
		Synchronized_OBJ(T& _Synchronized_obj, std::mutex& _myMutex)
			:Synchronized_obj(_Synchronized_obj), myMutex(_myMutex)
		{}
		std::mutex& GetMutex() {
			return myMutex;
		}
	...
	void foo_SYNCHRONIZED_OBJ(int k)
	{
		...
		for (int i = 0; i < LOOP_TIMES; i++)
		{
			...
			{
				t++;
				std::cout << t << " in the thread: " << k << std::endl;
			}
		}
	}
...
#define SYNCHRONIZED_MEMBER_FUNCTION \
	try { \
		INJECT_VARIABLE(static std::mutex, obfuscatedMutexName) \
			 throw std::move(std::unique_lock<std::mutex>(Sync::Synchronized_OBJ<decltype(*this)>(*this, obfuscatedMutexName).GetMutex())); \
	} catch (std::unique_lock<std::mutex>&)
...
void test_SYNCHRONIZED_OBJ() {
	std::thread t1(foo_SYNCHRONIZED_OBJ, 1);
	std::thread t2(foo_SYNCHRONIZED_OBJ, 2);
	t1.join();
	t2.join();
}
...
void test_SYNCHRONIZED_MEMBER_FUNCTION() {
	...
	std::thread t1(____1, std::ref(a), 1);
	std::thread t2(____1, std::ref(a), 2);
	std::thread t3(____2, std::ref(a), 3);
	std::thread t4(____2, std::ref(a), 4);
	t1.join();
	t2.join();
	t3.join();
	t4.join();
