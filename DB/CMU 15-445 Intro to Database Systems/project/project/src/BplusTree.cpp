...
namespace DB::tree
{
    ...
    void BTree::BT_create() {
        root = allocate_node(page_t_t::ROOT);
        root->ref();
    }

    BTreePage* BTree::allocate_node(page_t_t page_t) {
        ...
        case page_t_t::INTERNAL:
            page_ptr = new InternalPage(page_t, page_id, disk_manager_, key_t_, str_len_);
            break;
        case page_t_t::LEAF:
            page_ptr = new LeafPage(page_id, disk_manager_, key_t_, str_len_);
        default:
            return nullptr;
        }
        page_ptr->ref();
